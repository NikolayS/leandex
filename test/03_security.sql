...
declare
  _has_usage boolean;
begin
  select has_schema_privilege(current_user, 'leandex', 'usage') into _has_usage;

  if not _has_usage then
    raise exception 'FAIL: Current user lacks usage privilege on leandex schema';
  end if;
...
begin
  ...
  begin
    perform leandex.get_index_bloat_estimates(
      (select database_name from leandex.target_databases where enabled = true limit 1) || '; drop table leandex.config; --'
    );
...
begin
  ...
  begin
    perform leandex._remote_get_indexes_info(
