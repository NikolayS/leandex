...
PGPASSWORD='your_password' \
  ./leandex uninstall -H <control_host> -U <admin_user> -C <index_pilot_control_db> --drop-servers
```
...
-- Re-enable schedules
select cron.schedule_in_database(
  'leandex_daily', '0 2 * * *', 'call index_pilot.periodic(real_run := true);', 'index_pilot_control'
);
```
